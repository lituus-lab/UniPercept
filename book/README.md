...
| `index.nim` | what a perceptual hash is for |
| `hashes.nim` | aHash, dHash, pHash, wHash — what each one summarises |
| `distance.nim` | Hamming distance, and what it does not mean |
| `search.nim` | BK-trees, for more than a pairwise comparison |
| `files.nim` | hashing an image file rather than a buffer |
| `choosing.nim` | picking a threshold, responsibly |

Each chapter is its own program, so nothing carries between them:
`distance.nim` and `search.nim` rebuild the gradient `hashes.nim` introduced.
