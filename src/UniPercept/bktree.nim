...
proc len*(t: BkTree): int =
  ## Number of nodes (distinct hashes) in the tree.
  t.nodes.len

proc isEmpty*(t: BkTree): bool =
  t.nodes.len == 0

proc initBkTree*(): BkTree =
  ## An empty tree.
  BkTree(nodes: @[])

proc insert*(t: var BkTree; h: Hash; id: int32) =
...
proc query*(t: BkTree; h: Hash; radius: int): seq[(int32, int)] =
