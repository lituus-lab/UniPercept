...
## The version, stated in eight places, checked to agree.
##
...
suite "one version, eight copies":
  let manifest = valueOf("UniPercept.nimble", "version", "\"", "\"")
  ...
  test "the Nim constant agrees":
    check UniPerceptVersion == manifest

  test "the C header agrees, macros and string alike":
    let parts = manifest.split('.')
    check valueOf("include/UniPercept.h", "UNIPERCEPT_VERSION_MAJOR", " ",
        "") == parts[0]
    check valueOf("include/UniPercept.h", "UNIPERCEPT_VERSION_MINOR", " ",
        "") == parts[1]
    check valueOf("include/UniPercept.h", "UNIPERCEPT_VERSION_PATCH", " ",
        "") == parts[2]
    check valueOf("include/UniPercept.h", "define UNIPERCEPT_VERSION ", "\"",
        "\"") == manifest

  test "the C ABI reports it":
    let source = readFile(Root / "src/UniPercept/c_api.nim")
    check "UniPerceptVersion" in source

  test "commitizen agrees":
    check valueOf(".cz.toml", "version", "\"", "\"") == manifest

  test "the Nim suite's own tripwire agrees":
    # tests/test_percept.nim pins the constant as a literal, so a bump that
    # forgets it fails there. It is only useful if it names the same number.
    check valueOf("tests/test_percept.nim", "UniPerceptVersion ==", "\"",
        "\"") == manifest
  ...
  test "the Python test expects it":
    check valueOf("py/tests/test_percept.py", "unipercept.version()", "\"",
        "\"") == manifest
