...
proc layerOfModule(modulePath: string, order: seq[string]): int =
  ...
  ## name against any path component, so `UniPercept/decode` resolves to the
  ## `decode` layer and a bare `c_api` to the `c_api` layer.
...
iterator requiredPackages(path: string): string =
  ## Package name of every `requires` line (a line may list several specs).
  for raw in readFile(path).splitLines:
    let line = raw.strip
